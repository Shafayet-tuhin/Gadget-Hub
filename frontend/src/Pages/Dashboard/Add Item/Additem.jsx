...
const AddItem = () => {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append('expiration', '0');  // Set expiration to 0 so the image won't auto-delete
    ...
    fetch(imageBB_Url, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          fetch('https://gadget-hub-one.vercel.app/menu', {
            method: 'POST',
            headers: {
    ...
    <div className='w-full px-4 md:px-8 lg:px-16 py-8'>
      ...
      <div>
        <form className='grid grid-cols-1 md:grid-cols-2 gap-4 p-4 md:p-[5.5rem] bg-[#F3F3F3] rounded-xl lg:mb-32' onSubmit={handleSubmit}>
          ...
          <div className='col-span-1 md:col-span-2'>
            <label className="label">
              <span className="label-text text-[#444444] text-xl font-semibold">Product Details</span>
            </label>
            <textarea placeholder="Enter Product Details" name="details" className="w-full text-xl h-[15rem] py-[1.6rem] px-[2.25rem] border rounded-lg resize-none" required />
          </div>
